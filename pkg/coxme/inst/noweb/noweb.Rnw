...
named ones when they are referenced.  A named chunk is inserted
with the same relative indentation as the reference had.
...
<<tab.to.blank>>=
tab.to.blank <- function(x) {
    temp <- strsplit(x, '')
    blanks <- c("       ", "      ", "     ", "    ", "   ", "  ", " ", "")
    unlist(lapply(temp, function(x) paste(ifelse(x!='\t', x,
                                                 rep(blanks,length=length(x))),
                                          collapse='')))
    }
