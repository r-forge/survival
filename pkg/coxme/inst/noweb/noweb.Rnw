...
processing. The codestart vector gives the start of all code objects, and
the textstart one the start of all text objects.
...
nwread <- function(file) {
    ...
    endline <- c(temp[1,-1], length(program)) -1

    output <- vector('list', ncol(temp))  #number of code chunks
    oname <- rep('', ncol(temp))
...
nwparse <- function(lines) {
    ...
    if (length(indx)) {
        ...
        out <- list(lines=lines, xref=xref, indent=indent)
        }
...
nwextract<- function(code, target, prefix='') {
    ctemp <- code[[target]]
    if (is.null(ctemp)) stop("Program logic flaw 1")
    
    if (length(ctemp$xref) ==0) paste(prefix, ctemp$lines)
    else {
        temp <- vector('list', length(ctemp$xref))
        for (i in 1:length(ctemp$xref))
            temp[[i]] <- nwextract(code, ctemp$xref[i], ctemp$indent[i])
        newlines <- vector('character', length=length(ctemp$lines) +
                           length(unlist(temp)) - length(temp))

        k1 <- k2 <- 0  #k1= last inclusion line, k2=output counter
        for (i in 1:length(ctemp$xindex)) {
            j <- ctemp$xindex[i]
            if (j> (k1+1)) {
                indx <- seq(k1+1, j-1)
                newlines[k2 + 1:length(indx)] <- ctemp$lines[indx]
                k2 <- k2 + length(indx)
                }
            newlines[k2 + 1:length(temp[[i]])] <- temp[[i]]
            k2 <- k2 + length(temp[[i]])
            k1 <- j
            }
        if (length(lines) > k1) {
            indx <- seq(k1+1, length(lines))
            newlines[k2+ 1:length(indx)] <- ctemp$lines[indx]
            }
        paste(prefix, newlines)
        }
    }
@ 
