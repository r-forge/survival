...
The code below initial rescaled $X$, but I later realized that this causes
problems for the [[refine.n]] code in the main program; it saw the rescaled
$X$ matrix but not the rescaled coefficients.  
Currently we rescale the default starting estimates, thus if a user replaces
