...
generate= function(newtheta, parms) {
    ...
    <<coxvarFull-generate-2>>
    <<coxvarFull-generate-3>>
    <<coxvarFull-generate-4>>
    }
@ 
...
if (nvar==0) 
    return(list(imap=imap, X=NULL, 
                ...
                parms=list(varlist=varlist, theta=theta, levels=levellist,
...
if (intercept)
    list(theta=log(theta[!which.fixed]), imap=imap, X=xnew, xmap=xmap, 
             parms=list(theta=theta, fixed=which.fixed, 
                        ...
                        xstd=xstd, xmean=xmean, intercept=intercept,
...
if (parms$case==4) {
    ...
    for (i in 1:nvar) {
        ...
        rmat[1:n1 + irow, 1:n1 + icol] <- addup(xvar)
        # covariate-covariate
        if (i<nvar) {
            ...
            for (j in (i+1):nvar) {
                ...
                rmat[1:n1+ irow, 1:n1 + icol] <- addup(zcov)
...
wrapup <- function(theta, b, parms) {
    ...
    if (parms$case==4) {
        ...
        for (i in 1:ngroup) {
            if (intercept) 
                random[[i]] <- split(random[[i]], 
                                     rep(c("Intercept", parms$xname),
                                           each=parms$nlevel[i]))
            else random[[i]] <- split(random[[i]],
                                      rep(parms$xname, each=parms$nlevel[i]))
            }

        # Reformat theta for printing: a list with one element per
        #  group, containing a matrix or single element
        parms.per.group <- length(theta)/ngroup
        if (parms.per.group ==1) {
            newtheta <- as.list(theta)
            names(newtheta) <- names(parms$nlevel)
            }
        else {
            newtheta <- vector('list', ngroup)
            names(newtheta) <- names(parms$nlevel)
            ...
            for (i in 1:ngroup) 
                newtheta[[i]] <- tfun(newtheta[1:parms.per.group + 
                                         parms.per.group*(i-1)])
            }
