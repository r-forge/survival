...
if (sum(block.sparse) <=1) {#make them all ordinary matrices
    for (1 in 1:ngroup) varlist[[i]] <- as.matrix(varlist[[i]])
    }
else {
...
(I haven't thought of a good idea for naming the correlations.) %'
...
theta <- rep(c(.2, .2/xstd, rep(0., nvar*(nvar+1)/2)), ngroup)
...
if (parms$case==4) {
    ...
    tindx <- seq(0, by=theta.per.group, length=ngroup)
    ...
    # upper left corner
    itheta <- theta[1+tindx]
    corner <- addup(theta[1+tindx])
    ...
    for (i in 1:nvar) {
        # diagonal
        rmat[1:n1+ i*n1, 1:n1 +i*n1] <- addup(theta[1+i+tindx]) #diagonal
        if (ngroup==1) tmat <- itheta[1]*parms$vmat
        else {
            tmat <- itheta[1] * parms$varlist[[1]]
            for (i in 2:group) tmat <- tmat + itheta[i]*parms$varlist[[i]]
            }
