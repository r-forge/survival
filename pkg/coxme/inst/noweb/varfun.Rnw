...
G <- expand.nested(G)
ngroup <- ncol(G)
F <- matrix(0, nrow=nrow(G), ncol=ngroup)
nlevel <- sapply(G, function(x) length(levels(x)))
...
@ 
The parent routine will compute the actual design matrix $Z$ from $X$ and
the groupings.  
It will have one regression column for each covariate 
...
\left( \begin{array}{ccc}
  ...
  \sigma_{13}I & \sigma_{23}I &  \sigma_3^2 I\end{array} \right)
\label{eq:varcov}
\end{equation}
...
following way:
for each level of the grouping variable we have an 
(nvar +1) by (nvar +1) set of variances/covariances, with the
intercept as the first column.
We will let theta have this structure.
The [[tname]] vector gives names to \emph{most} of the elements, in
...
<<coxvarFull-init-4>>=
theta <- rep(c(.2, .2/sqrt(apply(X,2,var)), rep(0., nvar*(nvar+1)/2)), ngroup)
tname <- outer(c("", paste(parms$xname, '/',sep=''), rep('/',nvar*(nvar-1)/2)),
               names(G), paste, sep='')
tname <- c(tname)
...
if (parms$case==4) {
    ...
    vmat <- (cmat* sqrt(variance)) diag(sqrt(variance))   
...
'institution/sex', each of which has the variance for the intercept, for
age, and then the correlation (appropriately labeled).
(We are echoing the desired form for the printout).
The random effect has the same form, containing the named portions of the
$b$ vector.

[[theta=list('institution'= theta[1])]] for the variances, where
[[theta[[1]]]] has been labeled with 'Intercept' as the term name,
and a similar list for the random effects, with each random effect
labeled by the group to which it belongs.
...
browser()
        ...
        indx <- rep(1:ngroup, parms$nlevel) #fixed effects
        indx <- c(indx, rep(indx, each=nvar*(nvar+1)/2))
        for (i in 1:ngroup) {
            tmat <- cmat[
