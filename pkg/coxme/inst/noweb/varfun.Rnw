...
As a default starting value we have std(b *X) = .1.
...
else theta <- .1 / mean(sqrt(apply(X,2,var)))
...
The default values for $\theta$ are 0 for the correlations and .1 for the
variances.  
For computation they are transformed with variances=$e^\theta$ and
correlations=$(e^\theta -1)/(e^\theta +1)$.
In the code below we set the intitial value to .1/std to start, then correct
it later to .1 when $X$ is rescaled.  This is so that any user inputs are in
...
if (intercept) theta.temp <- diag(c(.1, .1/xstd))
else           theta.temp <- diag(.1/xstd, nvar, nvar) #might be 1x1 !
