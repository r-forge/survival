...
if (nvar==0) {
    return(list(imap=matrix(as.numeric(G[,ncol(G)])), X=NULL, 
                theta=log(theta[!which.fixed]),
...
tmat <- diag(nvar+1)  #dummy variance/cov matrix
...
if (parms$case==4) {
    ...
    nvar <- ncol(xmap)/ncol(imap)    #number of covariates
