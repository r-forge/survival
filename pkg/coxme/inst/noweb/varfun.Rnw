...
\begin{enumerate}
  ...
  \item Intercept and slope, single grouping [[(1 + x1| g1)]]
  \item Intercept and slope, nested effects [[(1 + x1 | g1/g2)]]
\end{enumerate}
...
if (!collapse) {
    ...
    else { # No sparse, so don't reverse the levels & annoy user
        nsparse <- 0
        for (i in 1:n.nest) {
            gtemp <- G[,i ,drop=T]
...
if (nvar >0 && is.null(G)) {
   list(theta=theta[!which.fixed], F=NULL, X=Z,
         parms=list(fixed=which.fixed, theta=theta, nvar=nvar,
                    gname=gname, case=4))
    }
@ 

<<coxvarFull-generate>>=
        diag(parms$nvar) * theta,
@ 
...
<<coxvarFull-init5>>=
        ...
        list(theta=theta[!which.fixed], F=F, X=X, 
             parms=list(theta=theta, fixed=which.fixed,
                        nvar=nvar, ngroup=ngroup, gname=gname, case=5))
        }
