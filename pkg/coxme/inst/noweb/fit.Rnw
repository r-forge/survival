...
express the result as a fraction of the integrated likelihood.
<<refine2>>=
if (refine.n > 0) {
    nfrail <- ncol(gkmat)
...
2Our last step before optimization is to set the starting value.
...
if (length(itheta)==0) iter <- c(0,0)
    ...
    while(index[1] <= nstart[1]) {
        ...
        if (is.null(bestlog) || ll < bestlog) {  # (optim is set up to minimize)
            bestlog <- ll
            besttheta <- theta
          }
