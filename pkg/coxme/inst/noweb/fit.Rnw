...
is added to the dense portion of the first matrix, and the first is turned
into a single large block, if necessary.
<<define-penalty>>=
kfun <- function(theta, varlist, vparm, ntheta,
    ...
    if (!inherits(tmat, 'bdsmatrix')) 
        tmat <- bdsmatrix(blocksize=nrow(tmat), blocks=tmat)
...
form with an [[rmat]] slot of dimension $124 \times 64$, call this $T$.
...
R= \left( \begin{array}{ccccc}
             T[1--62, 1--2] & 0 & 0 &T[1--62, 63-128] &0\\
              0 & U &0 & 0 & 0 \\
...
if (is.null(control$sparse.calc)) {
    ...
    rcol <- max(fmat) - nsparse  #number of non-sparse intercepts
    
    if ((2*n) > (nevent*(nsparse-rcol))) control$sparse.calc <- 0
...
if (length(theta)) {
    ...
    mfit <- do.call('optim', c(list(par= theta[!fixed], fn=logfun, gr=NULL), 
                           optpar, logpar))
    theta <- mfit$par
    iter <- mfit$counts[1] * c(1, coxme.control$inner.iter)
    }
else iter <- c(0,0)
...
<<coxme-fit>>=
    ...
    fit <- .C(ofile,
              ...
              beta = as.double(c(rep(0., nfrail), fit0$coef*scale)),
              loglik = double(2),
              as.double(ikmat@blocks),
              as.double(ikmat@rmat),
              hdet = double(1))
...
<<coxme-rescale>>=
        ...
        if (nvar ==1 ) {
            ...
            temp <- temp-nfrail          #multiply cols
...
any fixed parameters, label the vector.
<<finish-random>>=
newtheta <- NULL   
nrandom <- length(varlist)
sindex <- rep(1:nrandom, ntheta) #which thetas to which terms
