...
<<coxfit6a-call>>=
dummy <- kfun(theta, varlist, vparm, nsparse, ntheta)
...
<<define-logfun>>=
logfun <- function(theta, varlist, vparm, kfun, nsparse, ntheta,
                   init, fit0, iter, ofile) {
    gkmat <- gchol(kfun(theta, varlist, vparm, nsparse, ntheta))
...
coxme.fit <- function(x, y, strata, offset, ifixed, control,
			...
			fmat, zmat, varlist, vparm, theta, nsparse,
                        ntheta, refine.n) {
    time0 <- proc.time()
...
<<define-logfun>>
if (length(theta)) {
    logpar <- list(varlist=varlist, vparm=vparm, nsparse=nsparse,
                   ntheta=ntheta, kfun=kfun,
                   init=c(rep(0., npenal), fit0$coef),
                   fit0= fit0$loglik[2],
