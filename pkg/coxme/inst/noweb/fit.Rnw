...
kfun <- function(theta, varlist, vparm, 
                 fcount=tapply(nfac.nevel,whichterm, sum), 
                 zcount = nslope) {

    nrandom <- length(varlist)
    if (nrandom == 1) return(varlist[[1]]$generate(theta, vparm[[1]]))
@ 
...
<<define-penalty>>=
    ...
    for (i in 1:nrandom) {
        temp <- varlist[[i]]$generate(theta[sindex==i], vparm[[i]])
...
<<define-logfun>>=
logfun <- function(theta, varlist, vparm, kfun,
                   init, fit0, iter, ofile) {
    gkmat <- gchol(kfun(theta, varlist, vparm))
...
coxme.fit <- function(x, y, strata, offset, ifixed, control,
			...
			fmat, zmat, varlist, vparm, theta, ntheta,
                        refine.n) {
    time0 <- proc.time()
