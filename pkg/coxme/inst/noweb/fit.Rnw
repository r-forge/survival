...
\end{description}
The routine simulates the difference between the true integrand and the
Laplace approximation of the integrand.  

<<refine>>=
if (refine.n > 0) {
    rbeta <- fit$beta[1:ncol(gkmat)]  #random coefs
    bmat <- matrix(rnorm(length(rbeta)*refine.n), ncol=refine.n)
...
#              init=ifixed, iter=0)
fit0 <- coxph.fit(x,y, strata=strata, 
