...
regular metrix from the remainder; the construction
[[(as.matrix(tmat))[k,k]]] would create a temporary matrix of possible
vast proportions.  
...
[[exp(rfit$loglik) - exp[[fit$loglik + rterm1 - rterm2]].  
However, many of these will be huge negative numbers and the
exponentials would become exact zeros, leaving no digits of
precision in the difference.  We first scale (tmean), and finally
express the result as a fraction of the integrated likelhood.
<<refine2>>=
if (refine.n > 0) {
    nfrail <- ncol(gkmat)
