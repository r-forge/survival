...
one represented in \emph{b}lock \emph{d}iagonal \emph{symmetic} form,
...
Otherwise we have some nitpicky bookkeeping.  Not
...
regular metrix from the remainder.
<<define-penalty>>=    
...
The routine calculates the log-lik for a sucession of Cox models, each
one using one of the random draws as it's random effect.
The second part of the calculation is later in the codesince it needs [[hmat]],
...
The second two terms for the calculation are quadratic forms.  The
first three variables are needed with or without refinement.
Mathematically, the acual correction is the mean of 
[[exp(rfit$loglik) - exp[[fit$loglik -penalty + rterm1 - rterm2]].  
However, many of these will be huge negative numbers and would become
zeros, completely losing precision.  Even this may be large, so we
express it as a fraction of the integrated likelhood.
<<refine2>>=
nfrail <- ncol(gkmat)
...
if (refine.n > 0) {
    ...
    delta  <- fcoef - gkmat %*% bmat
    rterm3 <- rowSums((t(delta) %*% hmat[1:nfrail, 1:nfrail])^2)/2
    temp <- cbind(rfit$loglik, fit$loglik[2] +rterm2 - (penalty +rterm3))
    tmean <- mean(temp)
    escale <- exp(tmean-ilik)
    errhat <- exp(temp[,1]-tmean) - exp(temp[,2]-tmean)
    ilik  <- ilik + log(1+ escale*mean(errhat))
    r.correct <- escale *c(correction= mean(errhat), 
...
Now for the degrees of freedom, which is formula 5.16 of Therneau and Grambsch
...
<<create-output-list>>=
	...
	out <- list(coefficients = fit$beta[-(1:nfrail)]/scale, frail=fcoef, 
	     ...
	     loglik=c(fit0$log[1], ilik, fit$log[2]-penalty), var=hinv,
    ...
    if (refine.n>0) {
        ...
        if (FALSE) out$refine.debug <- list(loglik=rfit$loglik, escale=escale,
                                           bmat=bmat, errhat=errhat)
        }
    out
    }
@ 
