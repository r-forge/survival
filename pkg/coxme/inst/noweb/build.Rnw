...
each column of $Z$,  a vector [[theta]] containing
...
In general the varlist is a named list with one list element per
random term.  Each element of the list can be:
\begin{itemize}
  \item A matrix or list of matrices.  This is useful for genetic
    data in particular.
  \item A [[coxvar]] object or a function which generates a covar object.
\end{itemize}
...
which term.  If there is only one random term then [[varlist]] is
not required to be a list of one element.
...
if (missing(varlist)) {
    ...
    if (is.list(varlist)) && all(sapply(varlist, is.matrix)) {
        if (nrandom >1) stop("An unlabeled list of matrices is ambiguous when there are multiple random terms")
        else varlist <- list(varlist)
        }
    if (inherits(varlist, 'coxvar') || is.function(varlist)) 
        varlist <- list(varlist)  

    while(length(varlist) < nrandom) varlist <- c(varlist, coxvarFull())
    for (i in 1:length(varlist)) {
        if (is.function(varlist[[i]])) varlist[[i]] <- eval(varlist[[i]])
        if ((is.list(varlist[[i]]) && all(sapply(varlist, is.matrix))) ||
            is.matrix(varlist[[i]])) varlist[[i]] <- coxvarMlist(varlist[[i]])
        if (!inherits(varlist[[i]], 'coxvar'))
           stop("Invalid element of the variance list")
        }
    }
