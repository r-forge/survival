...
if (is.null(varlist)) {
    ...
    if (is.function(varlist)) varlist <- eval(varlist)
    if (class(varlist)=='coxvar') varlist <- list(varlist)
    else if (ismat(varlist))
        varlist <- list(coxvarMlist(list(varlist)))
    else {
        ...
        if (all(sapply(varlist, ismat))) {
            ...
            else varlist <- list(coxvarMlist(varlist))
            }
            ...
            for (i in 1:length(varlist)) {
                ...
                if (ismat(varlist[[i]]))
                    varlist[[i]] <- coxvarMlist(list(varlist[[i]]))
                if (class(varlist[[i]]) != 'coxvar') {
...
if (missing(variance)) variance <- vector('list', nrandom)
    ...
    if (length(variance) > nrandom) 
        stop (paste("Variance must be a list of length", nrandom))
