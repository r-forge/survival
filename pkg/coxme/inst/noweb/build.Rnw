...
(Why not call the returned variable [[b]]?  First because single letter
components can cause unexpected grief later, [[fit$b]] might end up
fetching something else that starts with a `b'.  Second because the
original coxme function used this.)
<<finish-up>>=
...
thought.  The current solution is not to allow factors on the
covariates.  The issue is that the downstream routine has to know
