...
%The variables [[nfac]] and [[nslope]] contain the number of columns
...
for (i in 1:nrandom) {
    ...
    theta <- c(theta, init$theta)
    ntheta[i] <- length(init$theta)
    ...
    if (i==1) nsparse <- init$sparse
    stemp[2] <- 1   #force no other sparse terms
