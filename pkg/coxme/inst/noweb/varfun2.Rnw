...
\end{enumerate}
I have had problems where each of these had to be relaxed.
...
coxvarMlist <- function(varlist, rescale=TRUE, pdcheck=TRUE,  positive=TRUE) {
    ...
    positive <- positive
    <<coxvarMlist-initialize>>
    <<coxvarMlist-generate>>
    <<covaraMlist-wrapup>>
    list(initialize=initialize, generate=generate, wrapup=wrapup)
    }
@ 
...
singularity, so we use a range of default initial values.
<<Mlist-initial-value>>=
...
if (length(initial) >0) {
    ...
    indx <- !is.na(initial) & initial !=0  #which to use
    if (any(indx)) theta[indx] <- initial[indx]
    }
...
if (length(postive)==1) positive <- rep(positive, ntheta)
...
Checking the matrices for validity is straightforward.  We use non-negative
definite rather than positive definite because identical twins generate
...
@ 
Wrapup is also simple. If there are multiple thetas they are
named as X.1, X.2, etc.
<<coxvarMlist-wrapup>>= 
wrapup <- function(newtheta, b, parms) {
        ...
        if (length(theta)>1) 
            names(theta) <- paste("X", 1:length(theta), sep='.')
        else names(theta) <- "X"
        theta <- list(theta)
