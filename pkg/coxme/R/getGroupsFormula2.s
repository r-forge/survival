# $Id: getGroupsFormula2.s,v 1.1 2003/08/09 21:48:42 therneau Exp $
#
# Updated code for getGroupsFormula.default
...
#
getGroupsFormula2 <- function(object) {
    form <- formula(object)
    ...
    temp <- tfun(temp)
    if (length(temp)==1) {
        ## no conditioning expression, as no | was found
        return(NULL)
        }
    else form[[2]] <- temp[[2]]
    form
