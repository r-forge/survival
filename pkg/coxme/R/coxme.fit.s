# $Id: coxme.fit.s,v 1.18 2006/08/01 19:26:44 therneau Exp $
#
...
coxme.fit <- function(x, y, strata, offset, init, control,
	...
	status <- y[,3]
        routines <- c('coxfit6a', 'agfit6b', 'coxfit6c', 'agfit6d')
        }
    else {
	if (length(strata) ==0) {
	...
	status <- y[,2]
        routines <- c('coxfit6a', 'coxfit6b', 'coxfit6c', 'coxfit6d')
        }
...
#    if (any(theta <0)) stop ("Invalid value  for theta")
    ...
    kfun <- substitute(function(theta, varlist) {
	...
	kmat
	}, list(bsize=varlist[[1]][[1]]@blocksize, rcol=sum(nfrail-nsparse)))

    # The "substitute" above allows me to set the bsize and rcol arguments
    #  to a value, not known till now, and not have to pass them as arguments
    #  through the nlmin() or optim() calling sequence.
    # We don't do it for varlist, because that is not a simple "plug in"
    #  number.  
    ...
    ifit <- .C(routines[1], 
               ...
               copy=c(F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,T,T,F,F,F))    
    means   <- ifit$means
    scale   <- ifit$scale
    ...
    if (refine.n > 0) {
        # Use simulation to improve the fit
        bmat <- matrix(rnorm(sum(nfrail)*refine.n), sum(nfrail), refine.n)
	#  For each usage, b will = bmat %*% gchol(\Sigma)  
        sim.pen <- colSums(bmat^2)/2   #This is b' \Sigma^{-1} b /2 
        }
    else {
        # These are created just to fill out an argument list later, but
        #  are never used
        bmat <- sim.pen <- NULL
        }
	...
	if (length(vinit)==1) vinit <- rep(vinit, length(tindex))
		...
		ilik <- 1+ fit$loglik[2] -
                ...
                if (refine.n > 0) {
                    rfit <- .C(routines[4],
                               ...
                               loglik = double(refine.n),
                               approx = double(refine.n))
                    errhat <- mean(exp(rfit$log - ilik) -
                                   exp(fit$loglik[2] + sim.pen - 
				        (rfit$approx +ilik)))
                    ilik = ilik + log(1 + errhat)
                    }
		-(ilik - fit0) 
		}
		...
		mfit <- nlminb(start= vinit, logfun,
			   ...
			   init=fit$beta, ofile=routines[2], kfun=kfun,
                           fit0=fit$loglik[1], iter=control$inner.iter,
                            refine.n=refine.n, bmat=bmat, sim.pen=sim.pen,
                               routines=routines)
		}
	    else {
       ...
       if (refine.n > 0) {
            ...
            errhat <- exp(rfit$loglik- ilik) - 
                      exp(fit$loglik[2] + sim.pen - (rfit$approx + ilik))
            old.ilik <- ilik  #this is for debugging/testing, remove later
            ilik = ilik + log(1 + mean(errhat))
            r.correct <- c(correction=mean(errhat), var=var(errhat)/refine.n)
            }
    ...
    # the second/third lines below will be removed later, it's extra stuff for
    #  testing/validation (it's not clear that we need errhat either)
    #if (refine.n>0) out<- c(out, list(errhat=errhat, refine=r.correct,
    #                        r.bmat =bmat, r.log=rfit$loglik, 
    #                        r.pen=sim.pen, r.approx=rfit$approx))
    if (refine.n>0) out<- c(out, list(errhat=errhat, refine=r.correct))
