...
Warning! In version 2.2 of the code there was an important change
which is not backwards compatible with older releases.
In short, I had an error in the methods.
$$
\begin{tabular}{r|cccc} 
  &\multicolumn{4}{c}{Method} \\
  &lme & lmer & coxme $<2.2$ & coxme $\ge 2.2$ \\ \hline
  fixed effects coefficients & fixef & fixef & fixef & fixef \\
  random effects coefficients& ranef & ranef &  & ranef \\
  random effects variances   & VarCorr & VarCorr & ranef & VarCorr \\
\end{tabular}
$$
At the same time the internal names of these objects changed in the 
output structure, again to make the code more parallel to lme.
For those who had referenced the results using \verb+fit$frail+
and/or \verb+fit$coef$random+, please use \verb+ranef(fit)+
and \verb+VarCorr(fit)}+ respectively.
To all users my sincere apologies for this oversight, which became
fully apparent when updating lmekin.
...
The random effects are considered to be Gaussian with 
mean zero and a variance matrix $\Sigma$ described by
...
Namely, linear models have an implied intercept term.
