...
The first task is to decide what data we need.  The response
is always needed, but is normally saved as a part of the 
model.  The covariate matrix [[x]] will be needed for all but
...
<<rsr-data>>=
need.x <- is.na(match(type, c('respone', 'deviance', 'working')))
...
Next we need to retrieve the distribution.  
...
The matrix returned from here contains columns for each of
$$
...
respect to all the others are zero so need not be returned.

The log-likelihood can be written as
\begin{eqnarray*}
L &=& \sum_{exact}\left[\log(f(z_i)) -\log(\sigma_i) \right] +
      \sum_{censored} \log \left( \int_{z_i^l}^{z_i^u} f(u)du \\
  &\equiv& \sum_{exact}\left[g_1(z_i) -\log(\sigma_i) \right] +
      \sum_{censored} \log \left(g_2(z_i^l, z_i^u) \\
...
if (type=='deviance') {
    ...
    if (type=='lcase')  rr<- rowSums(dfbeta*score)
    }
