...
but that routine does not do so as yet.  We need to
\begin{enumerate}
...
handy and is sufficient to prevent overflow of the exp function.
...
if (length(strat)) {
    ...
    if (any(is.na(match(newstrat, names(sfit$strata))))) 
...
Since any cluster() terms will be used in
further computation, we can 
subtract means from every numeric variable without worry.  Trying
to subtract a mean from the response generates an error,
``can't subtract Surv objects'', so avoid it.
The strata.keep variable name is to avoid a name conflict with the
strata() function. Remember that a user might have 2 strata() 
in a single model, and if so we need to combine them.
...
<<pcoxph-getdata>>=
    ...
    if (missing(newdata)) {
        ...
        mf <- model.frame(Terms, data=newdata, xlev=object$xlevels,
                           na.action=na.action)
    ...
    varn <- sapply(mf, is.numeric) & !sapply(mf, is.Surv) #numeric variables
    means <- rowsum(mf[,varn], strata.keep, reorder=TRUE)/
         as.vector(table(strata.keep))
    mf[,varn] <- mf[,varn] - means[match(strata.keep, row.names(means)),]
    x <- model.matrix(Terms2, mf, contrasts.arg=object$contrasts)[,-1, drop=FALSE]
    }
@ 
...
Finally, the computation.  First is the simple case of 
either a linear predictor or risk = exp(linear predictor).
<<pcoxph-simple>>=
