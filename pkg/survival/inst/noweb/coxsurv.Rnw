...
@
For backwards compatabliity: I allow someone to give an ordinary vector
...
if (!individual) {
    ...
    if (survtype==1) 
        surv <-cumprod(unlist(lapply(survlist, function(x) x$surv)))
...
<<survfit.coxph-compute>>=
    ...
    for (i in 1:ntarget) {
        ...
        varh[[i]] <- (cumsum(slist$varhaz[indx]) + 
                      rowSums((dt %*% object$var)* dt)) * newrisk[i]^2
        }
    varh <- unlist(varh)
