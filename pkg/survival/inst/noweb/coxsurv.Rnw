...
If the newdata argument is missing we used [[object$means]] as the
...
that for the Breslow method matches the Aalen survival estimate, etc.
...
if (missing(type)) {
    ...
    temp1 <- c("kalbfleisch-prentice", "aalen", "fleming-harrington")
    survtype <- match(match.arg(type, temp1), temp1)
    if (is.na(survtype)) stop("Invalid argument to 'method'")
    }
if (missing(vartype)) {
    vartype <- survtype
    }
else {
    temp2 <- c("greenwood", "tsiatis", "efron")
    vartype <- match(match.arg(type, temp2), temp2)
...
This includes the survival times, model matrix, 
case weights and strata.
...
\begin{verbatim}
  ...
  survfit(fit)
\end{verbatim}
The check for [[type]] of the same flavor; since [[coxph]] can only deal
...
if (is.null(x) || ncol(x)==0) { # a model with ~1 on the right hand side
    stop("Survival curves are not implimented for a NULL Cox model")
    }
...
if (individual) {
    ...
    if (length(temp$vars) >0) {
        strata2 <- factor(mf2[temp$vars], levels=levels(strata))
    ...
    if (missing(newdata)) {
        x2 <- matrix(0.0, nrow=1, ncol=length(object$means))
        offset2 <- 0
        }
    else {
...
newrisk <- exp(c(x2 %*% coef) + offset2 - mean(offset))
@ 
...
a few years ago.  
Computations are separate for each strata, but all have a different number
...
First, compute the baseline survival curves for each strata.
<<survfit.coxph-compute>>=
...
\begin{eqnarray*}
  H_2(t) &=& d'(t) V d(t) \\                                        %'
  d(t) = \int_0^t [\overline x(t) - z] dN(t)
\end{eqnarray*}
...
if (!individual) {
    ...
    varhaz <- unlist(lapply(survlist, function(x) x$varhaz))
    ...
    if (survtype==1) surv <- unlist(lapply(survlist, function(x) x$surv))
    else surv <- exp(-cumhaz)
                                    
    if (is.matrix(x2) && nrow(x2) >1) {  #more than 1 row in newdata
        surv <- outer(surv, newrisk, '^')
        temp <- cumsum(varhaz)
        varh <- matrix(0., nrow=length(temp), ncol=ncol(x2))
        for (i in 1:nrow(x2)) {
            dt <- outer(cumhaz, x2[,i], '*') - xbar
            varh[,i] <- (temp + rowSums((dt %*% object$var)* dt)) *
                newrisk[i]^2
            }
        }
    else {
        ...
        varh <- (cumsum(varhaz) + rowSums((dt %*% object$var)* dt)) * 
            newrisk^2
        }
@
...
<<survfit.coxph-compute>>=
    result <- list(n=as.vector(table(strata)), 
                   ...
                   n.event=nevent,
                   n.cencor=unlist(lapply(survlist, function(x) x$n.censor)),
                   surv=surv,
                   type='type')
    if (length(survlist) >1) {
...
subject was present, for this row of the new data.
<<survfit.coxph-compute>>=
else {
    ntarget <- nrow(x2)
    surv <- vector('list', npart)
    n.event <- n.risk <- n.censor <- varh <-  time <- haz
    stemp <- match(newstrata, ustrata)
    for (i in 1:ntarget) {
        slist <- survlist[[stemp[i]]]
        indx <- which(slist$time > newy[i,1] & slist$time <= newy[i,2])
        time[[i]] <- slist$time[indx] - newy[i,1]
        ...
        dt <-  outer(cumhaz, x2[,i]) - xbar
        varh[[i]] <- (cumsum(varhaz) + rowSums((dt %*% object$var)* dt)) * 
            newrisk^2
        }
    varh <- unlist(varh)
    
    if (survtype==1) surv <- cumprod(unlist(surv))
    else surv <- exp(-cumsum(ulist(haz)))
    result <- list(n=table(strata)[stemp[1]],
...
<<agsurv>>=
    ...
    varhaz <- switch(vartype,
                     nevent/(nrisk * 
                               ifelse(nevent>=nrisk, nrisk, nevent-nrisk)),
                     nevent/nrisk^2,
                     nevent* tsum$sum2)
    xbar <- switch(vartype,
                   (xsum/nrisk)*haz,
    ...
    result <- list(time=time, n.event=nevent, n.risk=nrisk, n.censor=ncens,
                   hazard=haz,
                   cumhaz=cumsum(haz), varhaz=varhaz, ndeath=ndeath, 
                   xbar=apply(matrix(xbar, ncol=nvar),2, cumsum))
    if (survtype==1) result$surv <- cumprod(km$inc)
    result
    }
@ 
...
if (!censor) {
    ...
    if (nstrata >1) {
        temp <- rep(levels(ustrata), each=result$strata)
