/* SCCS $Id: survreg5.c,v 1.2 2006-08-30 20:53:49 m015733 Exp $
/*
...
static int debug;
void survreg5(Sint   *maxiter,   Sint   *nx,       Sint   *nvarx, 
...
static double dolik(int n, double *beta, int whichcase) {
    ...
    */
    ...
    surv_callback(z, funs[0]);  /* treat them both as a vector */
    ...
    */
	...
	cox_callback(1, beta, upen, ipen, &logpen, zflag); 
	...
	cox_callback(2, beta+nf, upen, ipen, &logpen, zflag);
	loglik += logpen;
	if (pdiag==0) {
	    for (i=0; i<nvar; i++) {
		u[i+nf] += upen[i];
