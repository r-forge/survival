#SCCS $Id: print.survfit.s,v 4.15 2000-03-02 20:40:13 boos Exp $
print.survfit <- function(x, scale=1, 
	...
	for (i in unique(stemp)) {
	    who <- (stemp==i)
	    if (is.matrix(surv)) {
		temp <- pfun(x$strata.all[i], stime[who], surv[who,,drop=F],
			  x$n.risk[who], x$n.event[who],
			  x$lower[who,,drop=F], x$upper[who,,drop=F])
		x1 <- rbind(x1, temp)
	        }
	    else  {
		temp <- pfun(x$strata.all[i], stime[who], surv[who], 
