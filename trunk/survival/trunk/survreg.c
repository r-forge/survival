...
**      scale   - if =0, then estimate the scale, otherwise this specifies
**                  the scale.  If =0, then coef will be of length nvar+1.
**                  Note that beta will contain the log(scale).
**      doinit  - if >0, then beta does NOT contain initial values.  The
**                  routine will do a 'glm type' initialization.  If scale is
**                  to be estimated, however, beta still DOES contain a
**                  starting guess for that parameter.
...
**      jmat    - alternate estimate based on U'U
...
void survreg(maxiter, nx, nvarx, y, ny, covar2, offset2,
		 beta, scalex, u, imatx, jmatx, loglik, flag, eps,
		 deriv, do_init, dist)
long    *nx,
	*maxiter,
	*nvarx,
...
double  beta[],
	...
	*eps;
    ...
    if (scale==0) nvar = nvar+1; /*sigma too */
    imat = dmatrix(imatx, nvar, nvar);
    jmat = dmatrix(jmatx, nvar, nvar);
    newbeta = u+nvar;
    savediag= newbeta + nvar;
    ...
    if (*do_init>0) { /* set up a glm style initialization */
	if (debug>0) fprintf(stderr, "Glm init-- scale=%f\n", scale);
	if (scale==0) {
	    nvar--;
	    scale = exp(beta[nvar]);
	    }
	glm_init=1;
	for (i=0; i<nvar; i++) beta[i]=0;
	maxiter2 =0;
	rnewton(&maxiter2, n, nvar, beta, u, imat, jmat, loglik, *eps,
	...
	if (scale==0) {
		...
		eta += offset[person];
		temp += (time[person] - eta) * (time[person] -eta);
		}
	    /* Dodge the case where temp==0  */
	    beta[nvar] = log(.1*exp(beta[nvar]) + .9*temp/n);
	    nvar++;
	    }
	maxiter2 = *maxiter;
	}

    *flag = rnewton(&maxiter2, n, nvar, beta, u, imat, jmat, loglik, *eps,
		    sreg_g,  sreg_deriv, newbeta, savediag, debug);
    *maxiter = maxiter2;

    /*
    ** Do the deviance correction
    */
    if (scale==0) scale = exp(beta[nvar-1]);
    temp =0;
    for (person=0; person<n; person++) {
	if (status[person]==1) switch(*dist) {
	    case 1: best = -(1 + log(scale));
		    break;
	    case 2: best = -log(4*scale);   /*log(1/4) - log(sc..*/
		    break;
	    case 3: best = -log(SPI*scale);
		    break;
	    case 4: best = -log(PI*scale);
	    }
		...
		case 2: temp3= exp(temp2/2);
			...
			break;
		case 3: best = 2*erf(temp2/(2*ROOT_2));
			break;
		case 4: best = 2*atan(temp2/2)/PI  -1;
		}
	    }
	else best=0;

	g[person] = 2*(best - g[person]);
	temp      += best;
	}
    loglik[0] = 2*(temp - loglik[0]);
    loglik[1] = 2*(temp - loglik[1]);
    }


void sreg_g(n, nvar, beta, loglik)
int n, nvar;
double beta[];
double *loglik;
    {
    ...
    *loglik =0;
    ...
    for (person=0; person<n; person++) {
	if (glm_init==1) {
	    if (status[person]==3) eta= (time[person] + time2[person])/2;
	    else                   eta= time[person];
	    }
	else {
	    eta =0;
	    for (i=0; i<nvar; i++) eta += beta[i] * covar[i][person];
	...
	*loglik += g[person];
	if (glm_init==1) dg[person] -= eta*ddg[person];
	}
    }
...
*/
void sreg_deriv(n, nvar, beta, u, imat, jmat)
int     n;
int     nvar;
double  beta[],
	u[],
	...
	**imat;
	...
	for (i=0; i<nvar; i++) {
		...
		jmat[j][i] += covar[i][person] *covar[j][person] *dg[person]*
								  dg[person];
		}
	    }

	if (scale==0) {   /* need derivative wrt log sigma */
	    eta =0;
	    for (i=0; i<nvar; i++) eta += beta[i] *covar[i][person];
	    eta += offset[person];
		...
		for (i=0; i<nvar; i++) {
		    ...
		    jmat[i][nvar] += dsig[person]*dg[person]*covar[i][person];
		    }
		...
		for (i=0; i<nvar; i++) {
		    imat[i][nvar] -= (sz*ddg[person] - dg[person]) *
						covar[i][person];
		    jmat[i][nvar] += temp*dg[person] * covar[i][person];
		    }
		imat[nvar][nvar] -= sz*sz*ddg[person] - sz*dg[person];
		jmat[nvar][nvar] += temp*temp;
		}
	    }
	}
    }
